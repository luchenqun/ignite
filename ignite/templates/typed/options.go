...
func (opts *Options) ProtoPath(fname string) string {
	return filepath.Join(opts.AppPath, "proto", opts.AppName, opts.ModuleName, "v1", fname)
}

// ProtoTypeImport Return the protobuf import statement for this type.
func (opts *Options) ProtoTypeImport() *proto.Import {
	return protoutil.NewImport(fmt.Sprintf("%s/%s/v1/%s.proto", opts.AppName, opts.ModuleName, opts.TypeName.Snake))
}
